...
# symbols which are imported by "from hoyataxa.command import *"
...
from hoyataxa.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("hoyataxa"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="hoyataxa.config")
...
def bootstrap():
    ...
    By default this runs the function 'hoyataxa.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-hoyataxa = hoyataxa.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="HoyaTaxa %s" % version)
    ...
    _read_config(args)
    from hoyataxa.model import bootstrap_model
...
def start():
    ...
    from hoyataxa.controllers import Root
